//
// ASCOM.Simulator.CoverCalibrator Local COM Server
//
...
namespace ASCOM.Simulator
{
    public static class Server
    {
        ...
        private static void ElevateSelf(string arg)
        {
            ...
            {
                MessageBox.Show("The Simulator.CoverCalibrator was not " + (arg == "/register" ? "registered" : "unregistered") +
                    " because you did not allow it.", "Simulator.CoverCalibrator", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.ToString(), "Simulator.CoverCalibrator", MessageBoxButtons.OK, MessageBoxIcon.Stop);
            }
            return;
        }
        ...
        private static void RegisterObjects()
        {
            ...
            catch (Exception ex)
            {
                MessageBox.Show("Error while registering the server:\n" + ex.ToString(),
                        "Simulator.CoverCalibrator", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                return;
            }
            finally
            {
                ...
                switch (args[0].ToLower())
                {
                    ...
                    default:
                        MessageBox.Show("Unknown argument: " + args[0] + "\nValid are : -register, -unregister and -embedding",
                            "Simulator.CoverCalibrator", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        break;
                }
            }
            else
